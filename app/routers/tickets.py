...
router = APIRouter(prefix="/api/v1", tags=["T12-T13-p1"], dependencies=[Depends(require_api_key)])
...
def get_ticket(item_id: int, db: Session = Depends(get_db)):
...
def update_ticket(item_id: int, body: TicketUpdate, db: Session = Depends(get_db)):
    row = db.get(Ticket, item_id)
    if not row:
        raise HTTPException(404, detail="工单不存在")
...
def update_emergency(item_id: int, body: EmergencyUpdate, db: Session = Depends(get_db)):
